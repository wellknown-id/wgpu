...
cargo build --lib --target "$TARGET_TRIPLE" "${PROFILE_FLAG[@]}"
popd >/dev/null

WORK_DIR="$REPO_ROOT/target/$PROFILE/custom_webview_mobile_apk"
OUTPUT_APK="$REPO_ROOT/target/$PROFILE/apk/wgpu_example_04_custom_webview_mobile.apk"
...
"$AAPT2" link \
  --manifest "$SCRIPT_DIR/AndroidManifest-mobile.xml" \
  -I "$ANDROID_JAR" \
  -o "$WORK_DIR/base.apk"
